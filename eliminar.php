<?php
   ...
   if (!isset($_SESSION['Id_Usuario'])) {
    ...
    if ($Tipo_Usuario == 1) {
        ...
        $sqlE="DELETE FROM  estudiantes WHERE Codigo_Est = $id";
        $resultadoE = $mysqli-> query ($sqlE);
        ////////obtener nombre del proyecto
        if($id_pro>0){
            $sqlP0="SELECT * FROM  proyecto WHERE Cod_proyecto = $id_pro";
            $resultadoP0 = $mysqli-> query ($sqlP0);
            while ($row=$resultadoP0->fetch_assoc() ) {
                $id_pro = $row['Cod_proyecto'];
                $nom_pro = $row['Nombre_proyecto'];    
            }
            
            $sqlP="DELETE FROM  proyecto WHERE Cod_proyecto = $id_pro";
            $resultadoP = $mysqli-> query ($sqlP);
                        ////////////////eliminar archivos del estudiante/////
                        eliminarDir('documentos/'.$id);

        }
        $sqlU="DELETE FROM  usuarios WHERE Id_Usuario = $id";
        $resultadoU = $mysqli-> query ($sqlU);
        $sqlD="UPDATE docentes SET Codigo_Est='', Cod_proyecto='' WHERE Codigo_Doc = $id_doc";
