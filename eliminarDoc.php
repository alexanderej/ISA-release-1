...
///Se utiliza para detectar si el docente tiene asignado un estudiante
        $sql="SELECT * FROM  docentes WHERE Codigo_Doc = $id";
        $resultado = $mysqli-> query ($sql);

        while ($row=$resultado->fetch_assoc() ) {
            $id_est = $row['Codigo_Est'];   //Se obtiene el id del estudiante asignado si existe
        }
        
        $sqlD="DELETE FROM docentes WHERE Codigo_Doc = $id";///Eliminar el docente
        $resultadoD = $mysqli-> query ($sqlD);
    
        $sqlU="DELETE FROM  usuarios WHERE Id_Usuario = $id";///Eliminar docente de la tabla usuarios
